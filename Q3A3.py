...
query = '''SELECT COUNT(order_id), (COUNT(order_item_id)/COUNT(DISTINCT order_item_id)) as average FROM Order_items
           WHERE order_id IN (
...
def Uninformed():
    global query, connection, cursor
...
def Self_optimized():
    global cursor, connection, query
...
def stacked_bar_graph(first_stack, second_stack, third_stack):
    ...
    ax.set_title('Q3 (Runtime in ms)')
    ax.legend()

    path = './{}A3chart.png'.format('Q3')
    plt.savefig(path)
...
def main():
    ...
    # db_paths = ["./A3Small.db","./A3Medium.db"]
    uninformed_vals = []
    self_opt_vals = []
    user_opt_vals = []
